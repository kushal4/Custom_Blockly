...
Blockly.cake['controls_if'] = function(block) {
  ...
  do {
    conditionCode = Blockly.cake.valueToCode(block, 'IF' + n,
      Blockly.cake.ORDER_NONE) || 'false';
    branchCode = Blockly.cake.statementToCode(block, 'DO' + n);
    code += (n > 0 ? ' else ' : '') +
        'if (' + conditionCode + ') {\n' + branchCode + '}';
  ...
  if (block.getInput('ELSE')) {
    branchCode = Blockly.cake.statementToCode(block, 'ELSE');
    code += ' else {\n' + branchCode + '}';
  }
  return code + '\n';
};

Blockly.cake['controls_ifelse'] = Blockly.cake['controls_if'];
