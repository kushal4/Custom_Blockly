...
/**
 * Custom SVG path for drawing vertical tab
 * @type {string}
 */

Blockly.BlockSvg.CUSTOM_TAB_PATH_DOWN='v 8 c 0,10 '+Blockly.BlockSvg.TAB_WIDTH + ',-8 ' +Blockly.BlockSvg.TAB_WIDTH
+',7.5 s -'+Blockly.BlockSvg.TAB_WIDTH + ''+',-2.5 -' +Blockly.BlockSvg.TAB_WIDTH + ',7.5';

Blockly.BlockSvg.SH_INP_CUSTOM_TAB_PATH_UP='v -8 c 0,-10 '+Blockly.BlockSvg.TAB_WIDTH + ',8 ' +Blockly.BlockSvg.TAB_WIDTH
    +',-7.5 s -'+Blockly.BlockSvg.TAB_WIDTH + ''+',2.5 -' +Blockly.BlockSvg.TAB_WIDTH + ',-7.5';

Blockly.BlockSvg.LEFTVSHAPED_PATH=' 11,-13';
Blockly.BlockSvg.RIGHTVSHAPED_PATH=' l -11 15 11 15';
Blockly.BlockSvg.RIGHT_OUT_VSHAPE_PATH='l -11 -15';
/**
...
Blockly.BlockSvg.HIGHLIGHT_TOP_LEFT_CORNER_CHN_START='m 0,' + (Blockly.BlockSvg.CORNER_RADIUS-0.5);
/**
...
Blockly.BlockSvg.IN_CHAN_TOP_LENGTH=79.32550048828125 ;
/**
...
Blockly.BlockSvg.prototype.render = function(opt_bubble) {
  //console.log("to be rendered  now");
  ...
  var inputRows = this.renderCompute_(cursorX);
    //console.log("cursor x coordinate of type :- "+inputRows[0]["type"]+"is :-"+cursorX );
    ...
    if (row.type != Blockly.BlockSvg.INLINE) {
      if (row.type == Blockly.NEXT_STATEMENT) {
        ...
        if (row.type == Blockly.INPUT_VALUE) {
          ...
          hasDummy = true;
        } else if(row.type === Blockly.CUSTOM_INPUT_CHANNEL_SHAPE||
            row.type=== Blockly.SH_INP_CHANNEL_SHAPE){
            hasValue = true;
        }
...
Blockly.BlockSvg.prototype.renderDraw_ = function(iconWidth, inputRows) {
  ...
  //console.log("the draw of type :-"+inputRows[0]["type"] + "is :-"+iconWidth);
  ...
  if (this.outputConnection) {
    ...
    //console.log("the rightedge length is -:" +inputRows.rightEdge);
  ...
  this.renderDrawTop_(steps, highlightSteps, inputRows.rightEdge,connectionsXY,inputRows);
  var cursorY = this.renderDrawRight_(steps, highlightSteps ,inlineSteps,
      highlightInlineSteps, inputRows, connectionsXY,iconWidth);
  this.renderDrawBottom_(steps, highlightSteps, cursorY,inputRows);
  this.renderDrawLeft_(steps, highlightSteps,connectionsXY,inputRows);

  var pathString = steps.join(' ') + '\n' + inlineSteps.join(' ');
 // console.log("type :-"+inputRows[0]["type"]+"with svg string is -:  "+pathString);
...
Blockly.BlockSvg.prototype.renderMoveConnections_ = function() {
 ...
 // if(this.rightoutputConnection){
    //this.rightoutputConnection.moveToOffset(blockTL);
 // }
...
Blockly.BlockSvg.prototype.renderDrawTop_ =
    function(steps, highlightSteps, rightEdge,connectionXY,inputRows) {
  /* eslint-disable indent */
  ...
  //  console.log("curvy edge");
   // if(inputRows[0]["type"]!==Blockly.CUSTOM_INPUT_CHANNEL_SHAPE){
        steps.push(Blockly.BlockSvg.TOP_LEFT_CORNER_START);
      if(!this.setCustomBlockConnection){

          highlightSteps.push(Blockly.BlockSvg.HIGHLIGHT_TOP_LEFT_CORNER_CHN_START);
      }
        //highlightSteps.push()
    //  if(!this.setCustomBlockConnection) {
       //   highlightSteps.push(this.RTL ?
     ...
     // }
      ...
      if(!this.setCustomBlockConnection) {
      //  console.log("type came is "+inputRows[0]["type"]);
    ...
    //}
       ...
       // console.log("prev connection "+this.previousConnection);
  // Top edge.
  if (this.previousConnection) {
   // console.log("previous connection there");
  ...
  //console.log("top edge length is "+ rightEdge);
  //if()
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function(steps, highlightSteps,
    inlineSteps, highlightInlineSteps, inputRows,connectionXY ,iconWidth) {
  var cursorX;
  var cursorY = 0;
  ...
  for (var y = 0, row; row = inputRows[y]; y++) {
   ...
   // console.log(row);
    ...
    if(!this.setCustomBlockConnection){
      ...
      //if (this.outputConnection) {
         // if(row.type=== Blockly.CUSTOM_INPUT_CHANNEL_SHAPE ||
           //   row.type===Blockly.SH_INP_CHANNEL_SHAPE) {
            //  console.log("this is custom output");
          //}
     // }
    ...
    if (this.isCollapsed()) {
      ...
      this.width = Math.max(this.width, cursorX)+inputRows.rightEdge;
      ...
      if (this.RTL) {
       ...
       // console.log("row type input with type" + row.type);
      // External input.
        if(!this.setCustomBlockConnection){
            var input = row[0];
            var fieldX = cursorX;
            ...
            this.renderFields_(input.fieldRow, fieldX, fieldY);
            var v;
if(!this.RightLineconnection){
    steps.push(Blockly.BlockSvg.TAB_PATH_DOWN);
}

            v = row.height - Blockly.BlockSvg.TAB_HEIGHT;
            if(this.RightLineconnection){
                steps.push(Blockly.BlockSvg.RIGHTVSHAPED_PATH);
            }else{
                if(this.senseblockConnection){
                    //else{
                    if(this.LeftVerticalLen!== 0){
                        steps.push('v',this.LeftVerticalLen);
                    }
                    // }
           ...
           // console.log(input.name);
            //if(input.name!== "sense_conditioner"){
             // console.log()
         ...
         //   }else{
          ...
          // console.log(input);
           ...
           // console.log(this.type);
            ...
            if (this.outputConnection) {
                connectionX =  inputRows.rightEdge + 1;
                var input = row[0];
                var fieldX = cursorX + 5;
                var fieldY = cursorY;
                // console.log(input.align);
               // connectionX = connectionXY.x +  inputRows.rightEdge;
                // connectionY = connectionXY.y + inputRows. -
                ...
                this.renderFields_(input.fieldRow, fieldX, fieldY);
                steps.push(Blockly.BlockSvg.CUSTOM_TAB_PATH_DOWN);
                ...
                this.outputConnection.setOffsetInBlock(connectionX,cursorY);
            }
        }
  ...
  if(!this.setCustomBlockConnection){
   ...
   //console.log("highlight right "+highlightSteps);
  return cursorY;
};
 ...
 * @param {Object} inputRows object containing the current block object and its properties
 * @private
 */
Blockly.BlockSvg.prototype.renderDrawBottom_ =
    function(steps, highlightSteps, cursorY,inputRows) {
//  console.log(inputRows);
  /* eslint-disable indent */
 // if(inputRows[0]["type"]=== Blockly.CUSTOM_INPUT_CHANNEL_SHAPE){
...
//  }
  ...
  if (this.nextConnection) {
   // console.log("next connection there");
    ...
    this.height += 4;  // Height of tab.
        ...
        if(this.setCustomBlockConnection){
            ...
            //   inputRows.rightEdge + 1;
            // console.log(cursorY)
            //inputRows[0][0].connection.setOffsetInBlock(connectionX,cursorY);
            steps.push('h', -inputRows.rightEdge);
            if (!this.RTL) {
 ...
 * @private
 */
Blockly.BlockSvg.prototype.renderDrawLeft_ = function(steps, highlightSteps,connectionXY,inputRows) {
    //if(inputRows[0]["type"]=== Blockly.CUSTOM_INPUT_CHANNEL_SHAPE){

        //console.log("highlight left "+highlightSteps);

        // steps.push('l','-12,-10.4');
   // }
  //  console.log("render leftdone");
    var input = inputRows[0];
   // console.log(input);
  ...
  if (this.outputConnection) {
    if(!this.setCustomBlockConnection){
        ...
        if(this.LeftOutConnection){
           //steps.push(R)
            steps.push(Blockly.BlockSvg.RIGHT_OUT_VSHAPE_PATH);
        }else{
            if(this.senseblockConnection){
                steps.push('v',"-"+this.LeftVerticalLen);
            }else{
...
if(this.LeftLineconnection){

    steps.push('l',Blockly.BlockSvg.LEFTVSHAPED_PATH);

   // steps.push('l','-12,-10.4');
}else if(this.setLeftInpConnection){
steps.push(Blockly.BlockSvg.SH_INP_CUSTOM_TAB_PATH_UP);
   //connectionX = this.RTL ? -inputRows.rightEdge - 1 :
         // inputRows.rightEdge + 1;
    //console.log( input);
    if (input[0].connection.isConnected()) {
        this.width = Math.max(this.width, inputRows.rightEdge +
            input[0].connection.targetBlock().getHeightWidth().width -
            Blockly.BlockSvg.TAB_WIDTH + 1);
    }
   // var connectionX = -inputRows.rightEdge - 1;
  //  console.log(connectionX);
  //  var connectionY = (connectionXY.y + 4) -
  //     Blockly.BlockSvg.CORNER_RADIUS;
